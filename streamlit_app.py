# streamlit_app.py — Hair Product Recommender (Mobile-Friendly)
...
# -----------------------------------------------------------
# ✅ PINK GRADIENT THEME + MOBILE CSS
# -----------------------------------------------------------
st.markdown("""
<style>
.stApp {
...
# Title
# -----------------------------------------------------------
st.markdown('<div class="main-title"><h1>💇🏽‍♀️ Hair Product Recommender</h1><p>Find products that love your curls!</p></div>', unsafe_allow_html=True)
...
def build_corpus_row(row: pd.Series) -> str:
    ...
    def add_dup(prefix, value, times=2):
        v = str(value).strip()
        if v and v.lower()!="nan": tokens.extend([f"{prefix}-{v.replace(' ','-').lower()}"]*times)
    ...
    if ings: tokens += [w.strip().replace(" ","-").lower() for w in ings.split(",") if w.strip()]
    return " ".join(tokens)
...
# -----------------------------------------------------------
# Ratings UI (responsive)
# -----------------------------------------------------------
st.write("Rate a few products (1–10). You only need 2–3 ratings for results.")
...
if compact:
    ...
    for i,row in sample.iterrows():
        c0,c1,*rest = cols
        c0.write(row["name"]); c1.write(row["category"])
        if "price" in df.columns:
            c2,c3 = rest
...
def get_recommendations(rated_dict, top_n=8):
    if not rated_dict: return pd.DataFrame()
    rated_df = df[df["name"].isin(rated_dict.keys())]
    if rated_df.empty: return pd.DataFrame()
    ...
    user_vec = (weights @ X[idx,:]).reshape(1,-1)
    sims = cosine_similarity(user_vec, X).ravel()

    out = df.copy()
    out["score"] = sims
    out = out[~out["name"].isin(rated_dict.keys())]
...
if st.button("Get recommendations"):
    ...
    if len(rated)<1:
        ...
        if recs.empty:
            st.info("No candidates match your filters. Try loosening filters.")
        else:
